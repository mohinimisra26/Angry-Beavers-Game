...
// Testing the size funcitonality works
...
void test_list_small() {
    ...
    *v = VEC_ZERO;
    ...
    list_add(l, v);
    assert(list_size(l) == 5);
    assert(vec_equal(*((vector_t*)list_get(l, 0)), (vector_t) {2, 2}));
    assert(vec_equal(*((vector_t*)list_get(l, 1)), (vector_t) {3, 3}));
    assert(vec_equal(*((vector_t*)list_get(l, 2)), (vector_t) {4, 4}));
...
void test_list_large_get_set() {
    ...
    // for (size_t i = 0; i < LARGE_SIZE; i++) {
    //     assert(vec_equal(
    //         *((vector_t*)list_get(l, 0)), // might cause problems
...
void test_list_large_add_remove() {
    ...
    for (size_t i = 1; i <= LARGE_SIZE; i++) {
        size_t value = i-1; // changed this think it should be right.
    ...
    for (size_t size = 0; size <= max_size; size++) {
        ...
        for (access->index = 0; access->index < size; access->index++) {
            ...
            *((vector_t*)list_get(access->list, access->index)) = new_vector;
            assert(vec_equal(*((vector_t*)list_get(access->list,
              access->index)), new_vector));
        }
...
// Adds to capacity and ensures the list can be added to afterwards.
void test_full_add() {
    const size_t size = 3;
